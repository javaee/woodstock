 ...
 */
webui.@THEME_JS@.widget.labeledBase.prototype.getLabelProps = function(props) {
    return props;
};
 ...
 */
webui.@THEME_JS@.widget.labeledBase.prototype.getLabelClassName = function(ontop) {
    return null;
};

/**
 * Prepare the properties for a label subcomponent and establish
 * the label on top state.
 * @return {boolean} true if successful; otherwise, false.
 */
webui.@THEME_JS@.widget.labeledBase.prototype.postCreate = function () {
	...
	//
	var labelprops = this.getLabelProps({});
	this.prototypejs.extend(labelprops, this.label);

	this.label = this.widget.getWidgetProps("label", labelprops);
...
/**
 * The <code>labeledBase</code> widget supports the following properties.
 *
 ...
 * @return {boolean} true if successful; otherwise, false.
 */
webui.@THEME_JS@.widget.labeledBase.prototype.setProps = function(props, notify) {
    if (props == null) {
        return false;
    }
 ...
 * @private
 */
webui.@THEME_JS@.widget.labeledBase.prototype._setProps = function(props) {
	...
	this.common.setVisibleElement(this.brNode, props.labelOnTop);

	// Always remove the last label on top selector.
	// 
	this.common.stripStyleClass(this.labelContainer,
	...
	//
	this._lastLabelOnTopClassName = 
	    this.getLabelClassName(props.labelOnTop);

	// Add the "ontop" selector.
	//
	this.common.addStyleClass(this.labelContainer,
	    this._lastLabelOnTopClassName);
    }

    // Cases:
    // - called from postCreate
    //   + creating a label
    ...
    //   + creating a label 
    //       same as called from postCreate
