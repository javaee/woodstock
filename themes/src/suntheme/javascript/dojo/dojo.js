...
(function(){
	...
	if(typeof this.webui_@THEME@_config.djConfig == "undefined"){ // Woodstock: Modified for custom name space.
		this.webui_@THEME@_config.djConfig = {};
	}
	...
	var _config =/*===== webui_@THEME@_config.djConfig = =====*/{
		isDebug: false,
		libraryScriptUri: "",
		preventBackButtonFix: true,
	...
	for(var option in _config){
		if(typeof webui_@THEME@_config.djConfig[option] == "undefined"){
			webui_@THEME@_config.djConfig[option] = _config[option];
		}
	}
	...
	webui.@THEME@.dojo.locale = webui_@THEME@_config.djConfig.locale;
...
(function(){
	...
	webui.@THEME@.dojo.mixin(webui.@THEME@.dojo, {
		...
		if(webui_@THEME@_config.djConfig.cacheBust && d.isBrowser){
			uri += "?" + String(webui_@THEME@_config.djConfig.cacheBust).replace(/\W+/g,"");
		}
		try{
	...
	webui.@THEME@.dojo._callLoaded = function(){
		...
		if(typeof setTimeout == "object" || (webui_@THEME@_config.djConfig["useXDomain"] && d.isOpera)){
...
//					if(!webui_@THEME@_config.djConfig["baseUrl"]){
//						webui_@THEME@_config.djConfig["baseUrl"] = src.substring(0, m.index);
//					}
...
//						var cfgo = eval("({ "+cfg+" })");
//						for(var x in cfgo){
//							webui_@THEME@_config.djConfig[x] = cfgo[x];
//						}
//					}
...
//		}
		...
		// Woodstock: Append slash typically left by parsing script tag.
                if (webui_@THEME@_config.djConfig["baseUrl"].charAt(webui_@THEME@_config.djConfig["baseUrl"].length) != '/') {
			webui_@THEME@_config.djConfig["baseUrl"] += "/";
		}
		d.baseUrl = webui_@THEME@_config.djConfig["baseUrl"];
		...
		if(webui.@THEME@.dojo.isIE && (window.location.protocol === "file:")){
			webui_@THEME@_config.djConfig.ieForceActiveXXhr=true;
		}
		...
		d.locale = webui_@THEME@_config.djConfig.locale || (d.isIE ? n.userLanguage : n.language).toLowerCase();
		...
		d._xhrObj= function(){
			...
			var http = null;
			var last_e = null;
			if(!webui.@THEME@.dojo.isIE || !webui_@THEME@_config.djConfig.ieForceActiveXXhr){
	...
	if(document.addEventListener){
		...
		if(webui.@THEME@.dojo.isOpera|| (webui.@THEME@.dojo.isMoz && (webui_@THEME@_config.djConfig["enableMozDomContentLoaded"] === true))){
...
// webui.@THEME@.dojo.requireIf((webui_@THEME@_config.djConfig["isDebug"] || webui_@THEME@_config.djConfig["debugAtAllCosts"]), "webui.@THEME@.dojo.debug");
...
// webui.@THEME@.dojo.requireIf(webui_@THEME@_config.djConfig["debugAtAllCosts"] && !window.widget && !webui_@THEME@_config.djConfig["useXDomain"], "webui.@THEME@.dojo.browser_debug");
// webui.@THEME@.dojo.requireIf(webui_@THEME@_config.djConfig["debugAtAllCosts"] && !window.widget && webui_@THEME@_config.djConfig["useXDomain"], "webui.@THEME@.dojo.browser_debug_xd");

if(webui_@THEME@_config.djConfig.isDebug){
		webui.@THEME@.dojo.require("webui.@THEME@.dojo._firebug.firebug");
}

if(webui_@THEME@_config.djConfig.debugAtAllCosts){
	webui_@THEME@_config.djConfig.useXDomain = true;
...
//webui.@THEME@.dojo.require("webui.@THEME@.dojo._base.Deferred"); // Woodstock: Unused.
//webui.@THEME@.dojo.require("webui.@THEME@.dojo._base.json"); // Woodstock: Unused.
 // Woodstock: Unused.
//webui.@THEME@.dojo.require("webui.@THEME@.dojo._base.Color"); // Woodstock: Unused.



// // Woodstock: Unused.
 // Woodstock: Unused.
//webui.@THEME@.dojo.requireIf(webui.@THEME@.dojo.isBrowser, "webui.@THEME@.dojo._base.xhr"); // Woodstock: Unused.
//webui.@THEME@.dojo.requireIf(webui.@THEME@.dojo.isBrowser, "webui.@THEME@.dojo._base.fx"); // Woodstock: Unused.

(function(){
	if(webui_@THEME@_config.djConfig.require){
		for(var x=0; x<webui_@THEME@_config.djConfig.require.length; x++){
			webui.@THEME@.dojo["require"](webui_@THEME@_config.djConfig.require[x]);
		}
	}
})();
...
//Register any module paths set up in webui_@THEME@_config.djConfig. Need to do this
//in the hostenvs since hostenv_browser can read djConfig from a
//script tag's attribute.
(function(){
	var mp = webui_@THEME@_config.djConfig["modulePaths"];
	if(mp){
		for(var param in mp){
...
(function(){
	...
	if(webui.@THEME@.dojo.isIE){ 
		...
		if(!webui_@THEME@_config.djConfig._allow_leaks){
...
webui.@THEME@.dojo.i18n.normalizeLocale = function(/*String?*/locale){
	...
	//		the user agent's locale unless overridden by webui_@THEME@_config.djConfig.
...
webui.@THEME@.dojo.i18n._requireLocalization = function(/*String*/moduleName, /*String*/bundleName, /*String?*/locale, /*String?*/availableFlatLocales){
	...
	if(bundle){
		if(webui_@THEME@_config.djConfig.localizationComplete && bundle._built){return;}
...
(function(){
	...
	var extra = webui_@THEME@_config.djConfig.extraLocale;
	if(extra){
...
//webui.@THEME@.dojo.require("webui.@THEME@.dijit._base.focus"); // Woodstock: Unused.

//webui.@THEME@.dojo.require("webui.@THEME@.dijit._base.place"); // Woodstock: Unused.
//webui.@THEME@.dojo.require("webui.@THEME@.dijit._base.popup"); // Woodstock: Unused.
//webui.@THEME@.dojo.require("webui.@THEME@.dijit._base.scroll"); // Woodstock: Unused.

//webui.@THEME@.dojo.require("webui.@THEME@.dijit._base.bidi"); // Woodstock: Unused.
//webui.@THEME@.dojo.require("webui.@THEME@.dijit._base.typematic"); // Woodstock: Unused.
//webui.@THEME@.dojo.require("webui.@THEME@.dijit._base.wai");
//webui.@THEME@.dojo.require("webui.@THEME@.dijit._base.window"); // Woodstock: Unused.
...
//webui.@THEME@.dojo.require("webui.@THEME@.dojo.parser"); // Woodstock: Unused.
