if(!@JS_NS@._dojo._hasResource["@JS_NS@._dojo._firebug.firebug"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
@JS_NS@._dojo._hasResource["@JS_NS@._dojo._firebug.firebug"] = true;
@JS_NS@._dojo.provide("@JS_NS@._dojo._firebug.firebug");

@JS_NS@._dojo.deprecated = function(/*String*/ behaviour, /*String?*/ extra, /*String?*/ removal){
	// summary: 
...
@JS_NS@._dojo.experimental = function(/* String */ moduleName, /* String? */ extra){
...
// FIREBUG LITE
	...
	//			If you are new to Firebug, or used to the Dojo 0.4 @JS_NS@._dojo.debug, you can learn Firebug 
	...
	//		|	var @JS_NS@._base.config._djConfig = {isDebug: true, popup:true };
	// example:
	//		Option for console height (ignored for popup)
	//		|	var @JS_NS@._base.config._djConfig = {isDebug: true, debugHeight:100 };
...
if(
	...
	(
		(@JS_NS@._base.config._djConfig["noFirebugLite"] !== true)
	)
){
(function(){
	...
	openWin = function(){
		...
		/*HTMLstring+='<button onclick="(function(){ console.log(@JS_NS@._dojo.version.toString()); })()">Test Parent Dojo</button>\n';*/
	...
	function createFrame(){
		...
		if(@JS_NS@._base.config._djConfig.popup){
			_firebugWin = openWin();
			_firebugDoc = _firebugWin.document;
			@JS_NS@._base.config._djConfig.debugContainerId = 'fb';
			var containerHeight = "100%";
			
			// connecting popup
			_firebugWin.console = window.console;
			_firebugWin.@JS_NS@._dojo = window.@JS_NS@._dojo;

		}else{
			_firebugDoc = document;	
			var containerHeight = (@JS_NS@._base.config._djConfig.debugHeight) ? @JS_NS@._base.config._djConfig.debugHeight + "px" :"300px";
		}
		
		var styleElement = _firebugDoc.createElement("link");
		styleElement.href = @JS_NS@._dojo.moduleUrl("@JS_NS@._dojo._firebug", "firebug.css");
		...
		if(typeof @JS_NS@._base.config._djConfig != "undefined" && @JS_NS@._base.config._djConfig["debugContainerId"]){
			consoleFrame = _firebugDoc.getElementById(@JS_NS@._base.config._djConfig.debugContainerId);
		}
		if(!consoleFrame){
		...
		var closeStr = (@JS_NS@._base.config._djConfig.popup) ? "" : '    <a href="#" onclick="console.close(); return false;">Close</a>';
		...
		addEvent(_firebugDoc, @JS_NS@._dojo.isIE || @JS_NS@._dojo.isSafari ? "keydown" : "keypress", onKeyDown);
	...
	@JS_NS@._dojo.addOnLoad(createFrame);
	...
	function logFormatted(objects, className){
		...
		for(var i=0; i<ids.length; i++){
			...
			@JS_NS@._dojo.connect(btn, "onclick", function(){
				// hide rows
	...
	function onKeyDown(event){
		...
		if(timestamp > onKeyDownTime + 200){
			var event = @JS_NS@._dojo.fixEvent(event);
			var keys = @JS_NS@._dojo.keys;
	...
	function onSplitterMouseDown(event){
		if(@JS_NS@._dojo.isSafari || @JS_NS@._dojo.isOpera){
			return;
		}
	...
	getObjectAbbr = function(obj){
		...
		if(isError){
		...
		}else if(@JS_NS@._dojo.isArray(obj)){
			nm ="[";
			...
			nm+="]";
		}else if((!@JS_NS@._dojo.isObject(obj))||@JS_NS@._dojo.isString(obj)){
			nm = obj+"";
		}else{
			nm = "{";
			for(var i in obj){
	...
	addEvent(document, @JS_NS@._dojo.isIE || @JS_NS@._dojo.isSafari ? "keydown" : "keypress", onKeyDown);
	
	if(	(document.documentElement.getAttribute("debug") == "true")||
		(@JS_NS@._base.config._djConfig.isDebug)
	){
		toggleConsole(true);
	}
})();
