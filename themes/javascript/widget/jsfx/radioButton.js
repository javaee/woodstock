...
dojo.provide("webui.@THEME@.widget.jsfx.radioButton");

dojo.require("webui.@THEME@.widget.radioButton");

/**
 * This function is used to obtain data asynchronously.
 */
webui.@THEME@.widget.jsfx.radioButton = {
    /**
    ...
    processSubmitEvent: function(props) {
        ...
        DynaFaces.fireAjaxTransaction(
            ...
            render: props.id,
            replaceElement: webui.@THEME@.widget.jsfx.radioButton.submitCallback,
            xjson: {
                id: props.id,
                event: "submit"
            }
    ...
    submitCallback: function(id, content, closure, xjson) {
        ...
        // Publish an event for custom AJAX implementations to listen for.
        dojo.event.topic.publish(
            webui.@THEME@.widget.radioButton.submit.endEventTopic, props);
        return true;
    }
}

// Listen for Dojo Widget events.
dojo.event.topic.subscribe(webui.@THEME@.widget.radioButton.refresh.beginEventTopic,
    webui.@THEME@.widget.jsfx.radioButton, "processRefreshEvent");
dojo.event.topic.subscribe(webui.@THEME@.widget.radioButton.submit.beginEventTopic,
    webui.@THEME@.widget.jsfx.radioButton, "processSubmitEvent");
