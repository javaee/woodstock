 ...
 */
webui.@THEME@.widget.jsfx.dropDown = {
    /**
    ...
    processSubmitEvent: function(props) {
        ...
        (domNode) ? domNode : document.forms[0], {
            ...
            render: props.id,
            replaceElement: webui.@THEME@.widget.jsfx.dropDown.submitCallback,
            xjson: {
                id: props.id,
                event: "submit"
            }
    ...
    refreshCallback: function(id, content, closure, xjson) {
        ...
        dojo.event.topic.publish(
            webui.@THEME@.widget.dropDown.refresh.endEventTopic, props);
        return true;
    },
    ...
    submitCallback: function(id, content, closure, xjson) {
        ...
        // Publish an event for custom AJAX implementations to listen for.
        dojo.event.topic.publish(
            webui.@THEME@.widget.dropDown.submit.endEventTopic, props);
        return true;
    }
}

// Listen for Dojo Widget events.
dojo.event.topic.subscribe(webui.@THEME@.widget.dropDown.refresh.beginEventTopic,
    webui.@THEME@.widget.jsfx.dropDown, "processRefreshEvent");
dojo.event.topic.subscribe(webui.@THEME@.widget.dropDown.submit.beginEventTopic,
    webui.@THEME@.widget.jsfx.dropDown, "processSubmitEvent");
