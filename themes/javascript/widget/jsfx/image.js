...
webui.@THEME@.widget.jsfx.image = {
    ...
    processRefreshEvent: function(props) {
        ...
        new DynaFaces.fireAjaxTransaction(
            ...
            render: props.id,
            replaceElement: webui.@THEME@.widget.jsfx.image.refreshCallback,
            xjson: {
                id: props.id,
                event: "refresh"
            }
    ...
    refreshCallback: function(id, content, closure, xjson) {
        ...
        webui.@THEME@.widget.image.refresh.publishEndEvent(json);
        return true;
    }
}

// Listen for Dojo Widget events.
dojo.event.topic.subscribe(webui.@THEME@.widget.image.refresh.beginEventTopic,
    webui.@THEME@.widget.jsfx.image, "processRefreshEvent");
