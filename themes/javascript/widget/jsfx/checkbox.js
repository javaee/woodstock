...
dojo.provide("webui.@THEME@.widget.jsfx.checkbox");
 ...
 */
webui.@THEME@.widget.jsfx.checkbox = {
    /**
    ...
    processSubmitEvent: function(props) {
        ...
        DynaFaces.fireAjaxTransaction(
            ...
            render: props.id,
            replaceElement: webui.@THEME@.widget.jsfx.checkbox.submitCallback,
            xjson: {
                id: props.id,
                event: "submit"
            }
    ...
    submitCallback: function(id, content, closure, xjson) {
        ...
        // Publish an event for custom AJAX implementations to listen for.
        dojo.event.topic.publish(
            webui.@THEME@.widget.checkbox.submit.endEventTopic, props);
        return true;
    }
}

// Listen for Dojo Widget events.
dojo.event.topic.subscribe(webui.@THEME@.widget.checkbox.refresh.beginEventTopic,
    webui.@THEME@.widget.jsfx.checkbox, "processRefreshEvent");
dojo.event.topic.subscribe(webui.@THEME@.widget.checkbox.submit.beginEventTopic,
    webui.@THEME@.widget.jsfx.checkbox, "processSubmitEvent");
