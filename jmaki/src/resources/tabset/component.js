...
jmaki.namespace("@JMAKI_NS@.tabset");

/*
 * jMaki wrapper for Woodstock tabset widget.
 *
 ...
 *            or a reference to other widgets on the page ("refId").
 ...
 *            that tab to be rendered open in the tabset.  If the "id"
 *            property is omitted, it is set to the tabset id plus
 *            "_tab<index>".  For refId, any referenced widgets must be
 *            rendered on the page before the tabset widget is rendered.
 *            If a tab object "closeable" property is true, a close
 *            button is added after that tab's title.
 ...
 *            default topic is "/woodstock/tabset".
 * subscribe: Topic to subscribe to for data model events; if not
 *            specified, the default topic is "/woodstock/tabset".
 ...
 *             topic: "/woodstock/tabset/select"}
 * 
 */
@JMAKI_NS@.tabset.Widget = function(wargs) {

    // Initialize basic wrapper properties.
    this._subscribe = ["/woodstock/tabset"];
    this._publish = "/woodstock/tabset";
    this._subscriptions = [];
...
// Create Woodstock widget.
@JMAKI_NS@.tabset.Widget.prototype._create = function(wargs) {

    // Get the jMaki wrapper properties for a Woodstock tabset.
    var props = {};
    if (wargs.args != null) {
    ...
    props.id = this._wid;
    props.widgetType = "tabset";

    // Create the Woodstock tabset widget.
    var span_id = wargs.uuid + "_span";
    @JS_NS@.widget.common.createWidget(span_id, props);
       
    // add event handling
    @JS_NS@.widget.common.subscribe(@JS_NS@.widget.tabset.event.selectedTopic,
	this, "_onTabSelected");    
    
};

// Unsubscribe from jMaki events and destroy the Woodstock widget.
@JMAKI_NS@.tabset.Widget.prototype.destroy = function() {
...
@JMAKI_NS@.tabset.Widget.prototype.postLoad = function() {
    // Do nothing...
};

// Callback function to handle tab selection event.
// Event payload: {targetId: "<tab_id>"}
@JMAKI_NS@.tabset.Widget.prototype._selectCallback = function(payload) {
...
@JMAKI_NS@.tabset.Widget.prototype._setContentCallback = function(payload) {
    if (payload && typeof payload.targetId == "string") {
...
@JMAKI_NS@.tabset.Widget.prototype._setIncludeCallback = function(payload) {
    if (payload && typeof payload.targetId == "string") {
...
@JMAKI_NS@.tabset.Widget.prototype._setRefidCallback = function(payload) {
    if (payload && typeof payload.targetId == "string") {
...
@JMAKI_NS@.tabset.Widget.prototype._setRefid = function(tabId, refid) {
    var widgetTab = @JS_NS@.widget.common.getWidget(tabId);
...
//		   topic: "/woodstock/tabset/select"}
@JMAKI_NS@.tabset.Widget.prototype._onTabSelected = function(props) {
...
// Converts jmaki model to props digestable by woodstock tabset widget
// Each "items" array object is mapped to an tabset tab widget.
// Within each items object, the property mapping:
//   id        -> id (default: widgetId + "_tab" + <index>)
//   label     -> title (default: "Tab <index>"
//   content   -> contents: [{content: <content_value>}]
//   include   -> contents: [{include: <include_url>}]
//   refId     -> contents: [{refId: <refid_value>}] (Woodstock only)
//   selected  -> selected
//   closeable -> closeable (Woodstock only)
//   lazyLoad  -> This property is ignored.
@JMAKI_NS@.tabset.Widget.prototype._convertModel = function(props){
	...
	for (var i = 0; i < props.items.length; i += 1) {
		...
		tab.contents.push(tabContentElement);
	    }
	    ...
	    if (item.closeable && item.closeable == true) {
		tab.closeable = true;
	    }
	    props.contents.push(tab);	
	}	// End of for
	props.items = null;
    }
};
